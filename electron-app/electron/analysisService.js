...
export async function analyzeAssessment(input) {
  ...
  if (!apiKey) {
    throw createError("API key missing. Open Settings and save a valid Gemini key first.");
  }
  ...
  const jobDescription = await resolveJobDescription(input.jobDescription, input.jobUrl);
...
async function resolveJobDescription(jobDescription, jobUrl) {
  ...
  if (trimmedDescription && trimmedUrl) {
    throw createError("Provide either a job description or a job URL, not both.");
  }
...
async function extractJobDescriptionFromUrl(jobUrl) {
  ...
  try {
    response = await fetch(jobUrl, {
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; JobFitDesktop/1.0; +https://localhost)",
      },
    });
  } catch (error) {
  ...
  if (!response.ok) {
    throw createError(`Could not fetch job URL: ${response.status} ${response.statusText}`);
  }
  ...
  if (text.length < 200) {
    throw createError("Could not extract enough job description text from the provided URL.");
  }
